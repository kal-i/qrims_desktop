...
Future<Response> _updateAdminApprovalStatus(
  ...
  try {
    ...
    final targetUser =
        await userRepository.getUserInformation(id: id);

    final adminApprovalStatus = json['admin_approval_status'] != null
        ? AdminApprovalStatus.values.firstWhere((e) =>
            e.toString().split('.').last == json['admin_approval_status'])
        : null;
    ...
    if (responsibleUser is SupplyDepartmentEmployee) {
      ...
      if (result == true) {
        ...
        return Response.json(
          ...
          body: {
            'message':
                'User $id admin approval status updated to $adminApprovalStatus.',
          },
        );
      } else {
        return Response.json(
